...
public class AuthenticationFilter implements Filter {
	...
	static {
		...
		PROTECTED_URLS.add("/new_student_step1");
		PROTECTED_URLS.add("/parent/new");
		PROTECTED_URLS.add("/parent/list");
		PROTECTED_URLS.add("/parent/delete");
		PROTECTED_URLS.add("/parent/exportExcel");
		PROTECTED_URLS.add("/adduser");
	}

	/**
	 * Xử lý yêu cầu HTTP. Kiểm tra và xác thực quyền truy cập vào các URL yêu cầu
	 * đăng nhập.
	 * 
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		// Bỏ chặn URL động có dạng Student?Id=
		if (relativeURI.startsWith("/student")) {
			// Kiểm tra nếu có tham số "Id" trong URL
			if (httpRequest.getParameter("astudS03") != null) {
				chain.doFilter(request, response); // Tiếp tục xử lý nếu tham số "Id" có
				return;
			}
		}

		// Bỏ chặn URL động có dạng parent/parentdetails?Id=
		if (relativeURI.startsWith("/parent/parentdetails")) {
			// Kiểm tra nếu có tham số "Id" trong URL
		...
		// Bỏ chặn URL động có dạng parent/parentEdit?Id=
		if (relativeURI.startsWith("/parent/parentedit")) {
			// Kiểm tra nếu có tham số "Id" trong URL
			if (httpRequest.getParameter("Id") != null) {
				chain.doFilter(request, response); // Tiếp tục xử lý nếu tham số "Id" có
				return;
		...
		if (PROTECTED_URLS.stream().anyMatch(relativeURI::startsWith)) {
			...
			httpResponse.sendError(HttpServletResponse.SC_NOT_FOUND, "Trang không tồn tại");
			return;
		}
	...
	/**
	 * Khởi tạo bộ lọc. Phương thức này không cần thực hiện thêm hành động gì trong
	 * trường hợp này.
	 * 
